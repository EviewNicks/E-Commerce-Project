...
$all_products = []; // Array untuk menyimpan semua produk berdasarkan kategori


foreach ($categories as $category) {
    // Abaikan kategori "Accessories"
    ...
    $formatted_products = array_map(function ($product) {
        return [
            ...
            "rating" => rand(3, 5), // Simulasi rating karena tidak ada di tabel
            "terjual" => rand(10, 50), // Simulasi penjualan
            "promo" => $product['promo'],
            "harga" => number_format($product['harga'], 0, ',', '.'),
            "jumlah_stok" => rand(10, 100), // Simulasi stok
            "image" => $product['image'],
        ];
    }, $products);

    // Tambahkan hasil produk ke kategori terkait
    $all_products[$category['name']] = $formatted_products;
...
<div class="col-span-12 flex flex-col items-center justify-end flex-shrink-0 pt-[88px]">
    <!-- Main Image -->
    <div
        class="relative overflow-x  main-image bg-White-Colors-2 flex  px-[80px] py-[14px] flex-col justify-center items-start gap-3 self-stretch">
        <?php foreach ($all_products as $category_name => $category_products): ?>
            ...
            <div class="flex flex-col items-start self-stretch">
                ...
                <?php if (!empty($category_products)): ?>
                    <!-- Box Product -->
                    <div class="box-content flex flex-nowrap items-center gap-2 self-stretch overflow-x-auto  scroll-snap-x snap-mandatory px-4 py-2 hide-scrollbar scrollbar-hide">
                        <?php foreach ($category_products as $product): ?>

                            <!-- Card Product -->
                            <a href="?page=productDescryption&id=<?= htmlspecialchars($product['id']) ?>&user_id=<?= $user_id ?>">
                                <div
                                    class=" relative card-product snap-start flex-shrink-0 shadow-lg group flex w-[200px] h-[256px] pt-4 px-3 pb-3 flex-col justify-between items-center rounded-3xl hover:cursor-pointer active:scale-95 active:shadow-inner transition-all">
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center justify-between self-stretch z-10">
                                            <p class="text-black font-poppins text-body-small">Rp. <?= $product['harga'] ?></p>
